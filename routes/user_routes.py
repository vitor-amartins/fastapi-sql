...
def user_router(get_db: callable):
    ...
    @router.post('/', status_code=status.HTTP_201_CREATED, response_model=User, tags=tags)
    def create_user(user: UserCreate, db: Session = Depends(get_db)):
        return user_handler.handle_create_user(db, user)

    @router.get('/', status_code=status.HTTP_200_OK, response_model=List[User], tags=tags)
    ...
    @router.get('/{user_id}', status_code=status.HTTP_200_OK, response_model=User, tags=tags)
    def get_user(user_id: UUID, db: Session = Depends(get_db)):
        return user_handler.handle_get_user(db, user_id)

    @router.delete('/{user_id}', status_code=status.HTTP_200_OK, response_model=bool, tags=tags)
    def delete_user(user_id: UUID, db: Session = Depends(get_db)):
        return user_handler.handle_delete_user(db, user_id)

    @router.patch('/{user_id}', status_code=status.HTTP_200_OK, response_model=User, tags=tags)
